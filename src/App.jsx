...
export default function App() {
    ...
    return (
        <div className="min-h-screen flex flex-col antialiased selection:bg-gray-700 selection:text-white">
            ...
            <header className="fixed w-full z-40 bg-black/80 backdrop-blur-md border-b border-gray-800 transition-all duration-300">
                <div className="container mx-auto px-6 py-4 flex justify-between items-center">
                    ...
                    <nav className="hidden md:flex space-x-8 items-center tracking-wider">
                        {['GALLERY', 'CONCEPT', 'ABOUT', 'CONTACT', 'LINKS'].map((item) => (
            ...
            {/* --- Links Section --- */}
            <section id="links" className="relative py-24 bg-gray-950 overflow-hidden">
                <div className="absolute inset-0 z-0">
                    <img src="images/moon.jpg" alt="Links" className="w-full h-full object-cover opacity-20 mix-blend-luminosity" />
                ...
                <div className="relative z-10 container mx-auto px-6">
                    <FadeInSection>
                        <div className="text-center mb-16">
                            <h2 className="text-xl font-medium brand-font tracking-[0.3em] text-gray-500">LINKS</h2>
                        </div>
                        
                        <div className="flex flex-wrap justify-center gap-10 items-center">
                            {[
                                { src: "images/logo2.png", alt: "design4qol", href: "https://design4qol.com" },
                                { src: "images/studioushio.png", alt: "Studio Ushio", href: "https://studioushio.design4qol.com" },
                                { src: "images/ephi.png", alt: "EPHI", href: "https://ephi.design4qol.com" },
                                { src: "images/savepoint_sq.png", alt: "Savepoint", href: "https://savepoint.design4qol.com" },
                                { src: "images/43s.png", alt: "43s", href: "https://43s.design4qol.com" }
                            ].map((link, i) => (
                                <a key={i} href={link.href} target="_blank" rel="noreferrer" className="group p-2 transition duration-500">
                                    <img src={link.src} alt={link.alt} className="h-10 md:h-12 w-auto object-contain grayscale opacity-40 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-500 transform group-hover:scale-110" />
                                </a>
                            ))}
                            
                            {/* Instagram */}
                            <a href="https://instagram.com/dark_side_luck" target="_blank" rel="noreferrer" className="group p-2 text-gray-500 hover:text-white transition-all duration-500 flex items-center justify-center transform hover:scale-110">
                                <Instagram size={36} strokeWidth={1.5} className="opacity-60 group-hover:opacity-100 transition-opacity" />
                            </a>
                        </div>
                    </FadeInSection>
                </div>
            ...
            {lightboxIndex !== null && (
                ...
                >
                    {/* 画像コンテナ：横に広がりすぎないように制御 */}
                    <div className="max-w-6xl w-full flex flex-col items-center justify-center pointer-events-none">
                        <img 
                            key={displayedPhotos[lightboxIndex].id} // 画像が変わるたびに確実に再描画させる
                            src={displayedPhotos[lightboxIndex].fullUrl} 
                            alt={displayedPhotos[lightboxIndex].title} 
                            className="max-h-[85vh] max-w-full object-contain shadow-2xl rounded-sm animate-in zoom-in-95 duration-300 pointer-events-auto"
                        />
                        <div className="mt-6 text-center animate-in slide-in-from-bottom-4 duration-300 pointer-events-auto">
                    ...
                    {/* コントロールボタン群：z-[60]を付けて確実に最前面に表示 */}
                    <button 
                        onClick={() => setLightboxIndex(null)} 
                        className="absolute top-6 right-6 text-gray-500 hover:text-white transition-colors p-2 z-[60]"
                    >
                    ...
                    <button 
                        ...
                        className="absolute left-2 md:left-8 text-white/40 hover:text-white p-2 md:p-4 transition-transform hover:-translate-x-2 z-[60]"
                    >
                        <ChevronLeft size={48} strokeWidth={1} className="w-10 h-10 md:w-12 md:h-12" />
                    </button>
                    
                    <button 
                        onClick={(e) => { e.stopPropagation(); setLightboxIndex((prev) => (prev + 1) % displayedPhotos.length); }} 
                        className="absolute right-2 md:right-8 text-white/40 hover:text-white p-2 md:p-4 transition-transform hover:translate-x-2 z-[60]"
                    >
                        <ChevronRight size={48} strokeWidth={1} className="w-10 h-10 md:w-12 md:h-12" />
                    </button>
                </div>
            )}
